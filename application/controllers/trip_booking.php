			...
			if(isset($_REQUEST['id'])){
				...
				if($res==true){
					...
					if($driver!=false ){
						...
						$gcm=$this->trip_booking_model->getDriverGcmRegIds(array("'".mysql_real_escape_string($app_key)."'"));
						$this->gcm->send_notification($gcm, $response);
					...
					if($drivers!=false){
						...
						for($i=0;$i<count($drivers);$i++){
							...
							if($app_key!=''){
								...
								$this->notification_model->updateNotifications($notificationdata,$notificationid);
								$app_keys[]="'".mysql_real_escape_string($app_key)."'";
						...
						$gcm=$this->trip_booking_model->getDriverGcmRegIds($app_keys);
						//print_r($gcm);exit;
						$this->gcm->send_notification($gcm, $response);
				...
				if($res==true){
					...
					if($driver!=false ){
							...
							$trips=$this->trip_booking_model->getDetails($conditon);
								...
								$time=explode(':',$trips[0]->pick_up_time);
								$unixtime=mktime($time[0],$time[1],0,$dates[1],$dates[2],$dates[0])*1000;
								$response['upt']=array('fr'=>$trips[0]->trip_from,'sec'=>$unixtime,'tid'=>$trips[0]->id,'to'=>$trips[0]->trip_to);
